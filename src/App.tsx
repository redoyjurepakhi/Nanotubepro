...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      if (diff > 0) {
        ...
        setPullDistance(d);
        if (d > 10 && e.cancelable) e.preventDefault();
      }
    };
  ...
  }, [pullDistance, activeTab, apiKeys]);
  ...
  const fetchVideos = async (query?: string, isAppend = false) => {
    ...
    try {
      ...
      if (apiKey) {
        ...
        let url;
        if (query) {
          url = `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=15&q=${encodeURIComponent(query)}&type=video,channel&key=${apiKey}&regionCode=${region}`;
        } else {
          // Diverse Feed Logic: Randomize between Trending, Category-specific, and topic searches
          const strategies = ["trending", "search", "category"];
          const strategy = strategies[Math.floor(Math.random() * strategies.length)];
          
          if (strategy === "trending") {
            url = `https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics,contentDetails&chart=mostPopular&maxResults=12&regionCode=${region}&key=${apiKey}`;
          } else if (strategy === "search") {
            const keywords = ["tech", "music", "gaming", "nature", "science", "travel", "documentary", "educational", "coding", "minimalism", "design", "cinematic", "vlog", "fitness", "lifestyle", "animation", "art", "cooking"];
            const randomKeyword = keywords[Math.floor(Math.random() * keywords.length)];
            const pageTokens = ["", "CBQQAA", "CGQQAA", "CJYBEAA", "CKwBEAA", "CPoBEAA", "CPwBEAA", "CLoBEAA"]; 
            const randomPage = pageTokens[Math.floor(Math.random() * pageTokens.length)];
            url = `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=12&q=${randomKeyword}&type=video&key=${apiKey}&regionCode=${region}&pageToken=${randomPage}`;
          } else {
            // Category Strategy for diversity
            const categories = ["1", "10", "15", "17", "20", "23", "24", "25", "28"]; // Film, Music, Pets, Sports, Gaming...
            const cat = categories[Math.floor(Math.random() * categories.length)];
            url = `https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics,contentDetails&chart=mostPopular&maxResults=12&videoCategoryId=${cat}&regionCode=${region}&key=${apiKey}`;
          }
        }
      ...
      if (results.length === 0) {
        ...
        let url;
        if (query) {
          url = `https://pipedapi.kavin.rocks/search?q=${encodeURIComponent(query)}&filter=videos`;
        } else {
          // Diverse Feed for Piped
          const keywords = ["news", "relaxing", "live", "tutorial", "unboxing"];
          const randomKeyword = keywords[Math.floor(Math.random() * keywords.length)];
          url = Math.random() > 0.5 
        ...
        if (res.ok) {
          ...
          results = items.map(transformPiped).filter((v: Video) => v.id).sort(() => Math.random() - 0.5);
        }
      }
  ...
  }, [loading, loadingMore, videos.length, activeTab]);
  ...
  const addToPlaylist = (video: Video, name: string = "My Playlist") => {
    setPlaylists(prev => {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex items-center justify-between px-6 md:px-8 py-4 bg-[#0A0A0A] border-b border-border-dark shrink-0 z-[60] backdrop-blur-md bg-opacity-80">
          ...
          <div className="flex-1 flex justify-center px-4 relative">
            ...
            >
              <Search className="w-5 h-5 text-white/40 group-hover:text-brand-red transition-colors" />
            </button>
          </div>
        ...
        >
          ...
          <motion.div 
            ...
            animate={{ 
              ...
              y: loading ? 0 : (pullDistance - 40),
              rotate: pullDistance * 5
            }}
          >
            <div className={`p-2 rounded-full shadow-lg ${pullDistance > 50 || loading ? 'bg-brand-red' : 'bg-white/10 backdrop-blur-md'}`}>
  ...
  return (
    ...
    >
      <PlayerView 
        ...
        onExpand={onExpand}
        onClose={onClose} 
        related={videos.filter((v: Video) => v.id !== video.id)}
        initialTime={getWatchProgress(video.id)}
        onProgress={(t: number, v: Video) => addToHistory(v, t)}
        onWatchLater={toggleWatchLater}
        onAddToPlaylist={addToPlaylist}
        onQueue={addToQueue}
        onShare={shareVideo}
...
const PlayerView: React.FC<{ 
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className={isMinimized ? "flex items-center gap-3 p-2 h-full w-full bg-[#0A0A0A] cursor-pointer" : "fixed inset-0 bg-[#050505] z-[100] flex flex-col overflow-y-auto"}
      onClick={() => isMinimized && onExpand && onExpand()}
    >
      <div className={isMinimized ? "flex items-center gap-3 w-full h-full" : "max-w-[1400px] mx-auto w-full p-4 lg:p-8 space-y-8"}>
        {!isMinimized && (
          <button 
            onClick={onMinimize}
            className="flex items-center gap-3 text-white/30 hover:text-brand-red transition-all font-black text-[10px] uppercase tracking-[0.2em] italic group mb-8"
          >
        ...
        <div className={isMinimized ? "flex items-center gap-3 w-full h-full" : "grid grid-cols-1 lg:grid-cols-12 gap-10"}>
          {/* PLAYER SECTION */}
          <div className={isMinimized ? "w-24 aspect-video shrink-0 relative" : "lg:col-span-8 space-y-8"}>
            <div 
              ref={isMinimized ? null : containerRef}
              onMouseMove={isMinimized ? undefined : handleInteraction}
              onTouchStart={isMinimized ? undefined : handleInteraction}
              className={isMinimized 
                ? "w-full h-full bg-black rounded-lg overflow-hidden relative"
                : `aspect-video bg-black rounded-[2rem] overflow-hidden border border-white/5 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.8)] relative group ${isFullscreen ? 'rounded-none border-0' : ''}`
              }
            >
          ...
          {/* RELATED VIDEOS / MINIMIZED CONTROLS */}
          {isMinimized ? (
            <>
              <div className="flex-1 min-w-0 pr-2">
                <h3 className="text-sm font-bold text-white truncate leading-tight">{activeVideo.title}</h3>
                <p className="text-[10px] text-white/40 truncate">{activeVideo.channel}</p>
              </div>
...
// End of functional definitions (internal components removed or moved to Common.tsx)
